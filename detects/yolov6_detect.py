...
from detects.yolov6_anchor_generator import generate_anchors
from models.modules.yolov6_modules import Conv
...
class YOLOv6Detect(nn.Module):
    ...
    def __init__(self, subtype='yolov6_s', num_classes=80, in_channels=[128, 256, 512], num_anchors=1, num_layers=3,
                 use_dfl=False, reg_max=16, depth_mul=1.0, width_mul=1.0):  # detection layer
        super().__init__()
    ...
    def forward(self, x):
        ...
        if self.training:
            ...
            return torch.cat([pred_bboxes, torch.ones((b, pred_bboxes.shape[1], 1), device=pred_bboxes.device,
