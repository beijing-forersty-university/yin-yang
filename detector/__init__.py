...
from loss import quality_focal_loss
...
class YOLOX(nn.Module):
    ...
    def forward(self, imgs, targets=None, mode='infer', **kwargs):
        ...
        if mode == 'infer':
            ...
            batch_size, _, image_size, _ = imgs.shape
            # body_feats = self.backbone(imgs)
            # neck_feats = self.neck(body_feats)
            # out = self.head(neck_feats)
            backbone = EightTrigrams(image_size, batch_size)
            body_feats = backbone(imgs)
            head = YOLOXHead()
            out = head(body_feats)

            # losses = self.loss(out, targets["gt"])
            losses = quality_focal_loss(out, targets["gt"])
            # print(losses)
            ...
            if mode == 'val':
                ...
                for width, height, scale, pad, predict in zip(targets['width'], targets['height'], targets['scales'],
                                                              targets['pads'], predicts):
                    if predict is not None:
                        ...
                        outputs.append(
                            {"boxes": torch.from_numpy(bboxes_np), "labels": predict[:, 6],
                             "scores": predict[:, 4] * predict[:, 5]})
                    else:
                        outputs.append(
                            {"boxes": torch.empty((0, 4)), "labels": torch.empty((0, 1)),
