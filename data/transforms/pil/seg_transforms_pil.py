...
class RandomScaleCropLong(object):
    def __init__(self, size, scale=(0.08, 1.0), keep_ratio=True, fill=0, ignore_label=255, padding_mode='constant',
                 interpolation=InterpolationMode.BILINEAR):
        super().__init__()
        if isinstance(size, numbers.Number):
...
if __name__ == '__main__':
    ...
    trf = RandomScaleCropLong(size=[1024, 1024], scale=[0.5, 2.0], keep_ratio=True)

    # torch.save(sample, '/home/lmin/pythonCode/scripts/weights/segnext/sample.pth')
    sample = torch.load('/home/lmin/pythonCode/scripts/weights/segnext/sample.pth')

    img, target = sample['image'], sample['target']
    img_ = copy.deepcopy(img)
    boxes = target["boxes"]
    for box in boxes:
        cv2.rectangle(img_, (int(box[0]), int(box[1])), (int(box[2]), int(box[3])), (255,0,0), 1, 0)
    cv2.imwrite('img0.jpg', img_)
    ...
    for box in boxes:
        cv2.rectangle(img1_, (int(box[0]), int(box[1])), (int(box[2]), int(box[3])), (255,0,0), 1, 0)
    cv2.imwrite('img1.jpg', img1_)
    '''
    print('finished!')