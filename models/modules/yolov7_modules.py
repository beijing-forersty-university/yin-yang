...
class DownA(nn.Module):
    def __init__(self, c1, c2):
        ...
        self.branch1 = nn.Sequential(nn.MaxPool2d(kernel_size=2, stride=2),
                                    Conv(c1, c2, 1, 1))
        self.branch2 = nn.Sequential(Conv(c1, c2, 1, 1),
                                     Conv(c2, c2, 3, 2))
...
class EELAN(nn.Module):
    def __init__(self, c1, c2, c3):
        ...
        self.conv3 = nn.Sequential(Conv(c2, c2, 3, 1),
                                   Conv(c2, c2, 3, 1))
        self.conv4 = nn.Sequential(Conv(c2, c2, 3, 1),
                                   Conv(c2, c2, 3, 1))

        self.conv5 = Conv(c2*4, c3, 1, 1)
...
class FeatureFusion(nn.Module):
    def __init__(self, c1, c2):
        ...
        self.conv1 = Conv(c1, c2, 1, 1)
        self.conv2 = Conv(c1, c2, 1, 1)
        self.conv3 = Conv(c2, mid_c, 3, 1)
        self.conv4 = Conv(mid_c, mid_c, 3, 1)
        self.conv5 = Conv(mid_c, mid_c, 3, 1)
        self.conv6 = Conv(mid_c, mid_c, 3, 1)

        self.conv7 = Conv(c2 * 4, c2, 1, 1)
    ...
    def forward(self, x):
        ...
        x5 = self.conv4(x4)
        x6 = self.conv4(x5)
        cat_x = torch.cat([x1, x2, x3, x4, x5, x6], dim=1)
        return self.conv7(cat_x)
