...
from src.models.detects.fcos_detect import coords_fmap2orig
...
class GenTargets(nn.Module):
    ...
    def forward(self, inputs):
        ...
        return torch.cat(cls_targets_all_level, dim=1), torch.cat(cnt_targets_all_level, dim=1), \
...
class FCOSLoss(nn.Module):
    def __init__(self, strides=[8, 16, 32, 64, 128], limit_range=[[-1, 64], [64, 128], [128, 256], [256, 512], [512, 999999]]):
        super().__init__()
    ...
    def forward(self, preds, target_boxes, target_labels):
        ...
        cls_targets,cnt_targets,reg_targets = self.target_layer([preds, target_boxes, target_labels])
        '''
        inputs list
        [0]preds:  ....
