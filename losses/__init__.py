...
class BboxIoU(object):
    ...
    def ciou(self):
        ...
        """
        with torch.no_grad():
            arctan = torch.atan(self.predict_bboxes[..., 2] / self.predict_bboxes[..., 3]) - torch.atan(
                self.gt_bboxes[..., 2] / self.gt_bboxes[..., 3])
    ...
    def siou(self, theta=4):
        ...
        s_cw = torch.max(self.predict_bboxes[:, 0], self.gt_bboxes[:, 0]) - torch.min(self.predict_bboxes[:, 0],
                                                                                      self.gt_bboxes[:, 0])
        s_ch = torch.max(self.predict_bboxes[:, 1], self.gt_bboxes[:, 1]) - torch.min(self.predict_bboxes[:, 1],
                                                                                      self.gt_bboxes[:, 1])
        ...
        omiga_w = torch.abs(self.predict_bboxes[:, 2] - self.gt_bboxes[:, 2]) / torch.max(self.predict_bboxes[:, 2],
                                                                                          self.gt_bboxes[:, 2])
        omiga_h = torch.abs(self.predict_bboxes[:, 3] - self.gt_bboxes[:, 3]) / torch.max(self.predict_bboxes[:, 3],
                                                                                          self.gt_bboxes[:, 3])
